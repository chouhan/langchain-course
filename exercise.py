...
GROQ_API_KEY_ERROR_MESSAGE = 'Groq API Key has not been setup in .env file, which ideally is the best practice'
load_dotenv()
...
def implement_set_api_key(api_key):
    ...
    if api_key:
        print(f"This is my groq api key: {api_key}")
        return
    
    os.environ["GROQ_API_KEY"] = GROQ_API_KEY_ERROR_MESSAGE
    print(f"This is my groq api key now, since I did not find it in .env file: {GROQ_API_KEY_ERROR_MESSAGE}")
    pass
...
def check_api_key():
    ...
    if "GROQ_API_KEY" not in os.environ or GROQ_API_KEY == GROQ_API_KEY_ERROR_MESSAGE:
...
def implement_query_model(llm, prompt):
    ...
    """
    # TODO: Your implementation herep
...
def implement_compare_models(prompt):
    ...
    return { 
        "llama4": implement_llama_4_model().invoke([prompt]).content,
        "llama_3_3": implement_llama_3_3_model().invoke([prompt]).content
        }
    pass
